...
def load_config() -> Dict[str, Any]:
    ...
    if os.environ.get("BATCH_SIZE"):
        config["processing"]["batch_size"] = int(os.environ.get("BATCH_SIZE"))
    
    if os.environ.get("SCHEDULE_INTERVAL"):
        config["schedule"]["interval_seconds"] = int(os.environ.get("SCHEDULE_INTERVAL"))
    
    # Try to load from config file
    config_path = os.path.join("config", "processor_config.json")
    try:
...
def should_run_now(config: Dict[str, Any]) -> bool:
    ...
    # Get last run time
    last_run_path = os.path.join("data", "processor_last_run.txt")
    try:
